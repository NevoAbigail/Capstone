...
\begin{itemize}
\item[SR\refstepcounter{srnum}\thesrnum\label{SR1}:] Internal parts of locking mechanism shall be accessible and replaceable (see NFR12 in SRS). 
...
\item[SR\refstepcounter{srnum}\thesrnum\label{SR2}:] Product shall be adaptable and be able to fit a wide variety of external frames/bike racks (see NFR14 in SRS).
\end{itemize}
...
The following requirements have already been included in the SRS document, and are restated here for convenience: \\
 ...
 \noindent FR6: Lock must only be engaged/disengaged by the intended user(s). \newline 
