...
\item[\refstepcounter{acnum}\actheacnum\label{acPowerSignal}:] How the Arduino allows/disallows power to the load (electromagnet) I.e., the implementation of the switch (transistor). 
...
\item[\refstepcounter{acnum}\actheacnum\label{acMagnet}:] The size of the electromagnet. 
...
\item[\refstepcounter{ucnum}\uctheucnum\label{ucCircuit}:] The circuit devices (Arduino, battery and electromagnet). 
...
\item [\refstepcounter{mnum} \mthemnum \label{mWSC}:] Wireless Signal Connection Module 
\item [\refstepcounter{mnum} \mthemnum \label{mBS}:] Battery Status Module 
\item [\refstepcounter{mnum} \mthemnum \label{mLPS}:] Load Power Signal Module 
\item [\refstepcounter{mnum} \mthemnum \label{mL}:] Location Module 
\item [\refstepcounter{mnum} \mthemnum \label{mB}:] Battery Module 
\item [\refstepcounter{mnum} \mthemnum \label{mEM}:] Electromagnet Module 
...
\begin{description}
\item[Secrets:]The data structure for input parameters and how the values are inputted. The load and verify secrets are isolated to their own access programs, found in their respective submodules. 
\item[Services:]Receives input from the user, stores input and verifies that the input parameters comply with physical and software constraints. There is no need for input verification because the user's input options are restricted such that they cannot be invalid.
