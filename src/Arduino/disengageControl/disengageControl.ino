    ...
    if (central) {
        ...
        while (central.connected()) {
           ...
           if (commCharacteristic.written()) {
                if (commCharacteristic.value()==password) {   // has to match the set password
                    Serial.println("GREEN LED on");
                    digitalWrite(GREEN_PIN, LOW);         // LOW will turn the LED on
                    digitalWrite(TRANSISTOR_OUT, HIGH); //sets the TRANSISTOR_OUT pin ON

                    delay(15000); //waits for 15 seconds 
                    digitalWrite(GREEN_PIN, HIGH);
                    digitalWrite(TRANSISTOR_OUT,LOW); //sets the TRANSISTOR_OUT pin  off
                } else {                              // any other value (i.e., nothing, or the incorrect password)
